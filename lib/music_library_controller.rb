class MusicLibraryController
  ...
  def call 
    ...
    while input != 'exit'
      ...
      puts "To list all of the artists in your library, entr 'list artists'."
      puts "To list all of the genres in your library, enter 'list genre'."
  ...
  def list_artists
    Artist.all.sort_by(&:name).each.with_index(1) do |art, index|
      puts "#{index}. #(art.name)"
    end 
  end 
  ...
  def list_songs_by_artist 
    puts "Please entr the name of an artist"
    input = gets.strip
   ...
   def list_songs_by_genre
    puts "Please entr the name of a genre"
    input = gets.strip
